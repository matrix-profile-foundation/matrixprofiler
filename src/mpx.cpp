...
// @return a List
// [[Rcpp::export]]
List mpx_rcpp(NumericVector a, uint64_t window_size, double ez = 0.5, bool idxs = true,
  ...
  try {
    ...
    NumericVector ddf = 0.5 * (a[Range(window_size, n - 1)] - a[Range(0, n - window_size - 1)]);
    ddf.push_front(0);
    NumericVector ddg = (a[Range(window_size, n - 1)] - mmu[Range(1, profile_len - 1)]) + (a[Range(0, n - window_size - 1)] - mmu[Range(0, n - window_size - 1)]);
    ddg.push_front(0);
    ...
    try {
      for (IntegerVector::iterator diag = seq_diag.begin(); diag != seq_diag.end(); ++diag) {
        ...
        c = inner_product((a[Range(*diag, (*diag + window_size - 1))] - mu[*diag]), ww);
...
// [[Rcpp::export]]
List mpxab_rcpp(NumericVector a, NumericVector b, uint64_t window_size, bool idxs = true,
                bool euclidean = true, bool progress = false) {
  ...
  try {
    ...
    NumericVector ddf_a = 0.5 * (a[Range(window_size, a_len - 1)] - a[Range(0, a_len - window_size - 1)]);
    ddf_a.push_front(0);
    NumericVector ddg_a = (a[Range(window_size, a_len - 1)] - mmu_a[Range(1, profile_len_a - 1)]) + (a[Range(0, a_len - window_size - 1)] - mmu_a[Range(0, a_len - window_size - 1)]);
    ddg_a.push_front(0);
    NumericVector ddf_b = 0.5 * (b[Range(window_size, b_len - 1)] - b[Range(0, b_len - window_size - 1)]);
    ddf_b.push_front(0);
    NumericVector ddg_b = (b[Range(window_size, b_len - 1)] - mmu_b[Range(1, profile_len_b - 1)]) + (b[Range(0, b_len - window_size - 1)] - mmu_b[Range(0, b_len - window_size - 1)]);
    ddg_b.push_front(0);
    ...
    try {
      for (IntegerVector::iterator diag = seq_diag.begin(); diag != seq_diag.end(); ++diag) {
        ...
        c = inner_product((a[Range(*diag, (*diag + window_size - 1))] - mu_a[*diag]), ww);
        for (offset = 0; offset < off_max; offset++) {
          off_diag = offset + *diag;
      ...
      for (IntegerVector::iterator diag = seq_diag.begin(); diag != seq_diag.end(); ++diag) {
        ...
        c = inner_product((b[Range(*diag, (*diag + window_size - 1))] - mu_b[*diag]), ww);
        for (offset = 0; offset < off_max; offset++) {
          off_diag = offset + *diag;
...
struct MatrixProfileP : public Worker {
  ...
  MatrixProfileP(const NumericVector a, const uint64_t window_size, const NumericVector df, const NumericVector dg,
                 const NumericVector mmu, const NumericVector sig, const NumericVector ww,
                 Progress *p, NumericVector mp, IntegerVector mpi) :
    a(a), window_size(window_size), df(df), dg(dg), mu(mmu), sig(sig), ww(ww), p(p), mp(mp), mpi(mpi) {}
...
// [[Rcpp::export]]
List mpx_rcpp_parallel(NumericVector a, uint64_t window_size, double ez = 0.5, bool idxs = true,
                       bool euclidean = true, bool progress = false) {
  ...
  try {
    ...
    NumericVector df = 0.5 * (a[Range(window_size, n - 1)] - a[Range(0, n - window_size - 1)]);
    df.push_front(0);
    NumericVector dg = (a[Range(window_size, n - 1)] - mu[Range(1, profile_len - 1)]) + (a[Range(0, n - window_size - 1)] - mu[Range(0, n - window_size - 1)]);
    dg.push_front(0);
    ...
    MatrixProfileP matrix_profile(a, window_size, df, dg, mu, sig, ww, &p, mp, mpi);
...
struct MatrixProfilePAB : public Worker {
  ...
  // initialize from Rcpp input and output matrixes (the RMatrix class
  // can be automatically converted to from the Rcpp matrix type)
  MatrixProfilePAB(const NumericVector a, const NumericVector b, const uint64_t window_size,
                   ...
                   const NumericVector ww_a, const NumericVector ww_b, Progress *p,
                   NumericVector mp_a, NumericVector mp_b,
                   IntegerVector mpi_a, IntegerVector mpi_b) :
    a(a), b(b), window_size(window_size),
    df_a(df_a), df_b(df_b),
    dg_a(dg_a), dg_b(dg_b),
    mu_a(mu_a), mu_b(mu_b),
    sig_a(sig_a), sig_b(sig_b),
    ww_a(ww_a), ww_b(ww_b), p(p),
    mp_a(mp_a), mp_b(mp_b),
...
// [[Rcpp::export]]
List mpxab_rcpp_parallel(NumericVector a, NumericVector b, uint64_t window_size, bool idxs = true,
                         bool euclidean = true, bool progress = false) {
  ...
  try {
    ...
    // calculations slightly and allows us to avoid special "first line"
    // handling.
    NumericVector df_a = 0.5 * (a[Range(window_size, a_len - 1)] - a[Range(0, a_len - window_size - 1)]);
    df_a.push_front(0);
    NumericVector dg_a = (a[Range(window_size, a_len - 1)] - mu_a[Range(1, profile_len_a - 1)]) + (a[Range(0, a_len - window_size - 1)] - mu_a[Range(0, a_len - window_size - 1)]);
    dg_a.push_front(0);
    NumericVector df_b = 0.5 * (b[Range(window_size, b_len - 1)] - b[Range(0, b_len - window_size - 1)]);
    df_b.push_front(0);
    NumericVector dg_b = (b[Range(window_size, b_len - 1)] - mu_b[Range(1, profile_len_b - 1)]) + (b[Range(0, b_len - window_size - 1)] - mu_b[Range(0, b_len - window_size - 1)]);
    dg_b.push_front(0);
    ...
    MatrixProfilePAB matrix_profile(a, b, window_size,
                                    df_a, df_b,
                                    dg_a, dg_b,
                                    mu_a, mu_b,
                                    sig_a, sig_b,
    ...
    try {
      ...
      partial = true;
      Rcout << "Process AB terminated by the user successfully, partial results were returned.";
    } catch (...) {
      ::Rf_error("c++ exception (unknown reason)");
    }
    ...
    try {
      ...
      partial = true;
      Rcout << "Process BA terminated by the user successfully, partial results were returned.";
    } catch (...) {
      ::Rf_error("c++ exception (unknown reason)");
    }
