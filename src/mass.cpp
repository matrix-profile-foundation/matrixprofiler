...
List mass_weighted_rcpp(const ComplexVector data_fft,
  ...
  distance_profile =
      data_pre - 2 * (last_product - sumwy * data_mean) / data_sd + sumwy2;
  distance_profile[distance_profile < 0] = 0;

  return (List::create(Rcpp::Named("distance_profile") = distance_profile,
...
List mass_absolute_rcpp(const ComplexVector data_fft,
  ...
  try {
    NumericVector rev_query(
        data_fft.length()); // use data_fft because is padded to a power of 2
    std::reverse_copy(query_window.begin(), query_window.end(),
                      rev_query.begin());
    ...
    distance_profile =
        as<NumericVector>(sumx2[range_d]) - 2 * last_product + sumy2;
    distance_profile[distance_profile < 0] = 0;
...
List mass2_rcpp(const ComplexVector data_fft, const NumericVector query_window,
  ...
  try {
    ...
    distance_profile =
        2 *
        (window_size - (last_product - window_size * data_mean * query_mean) /
                           (data_sd * query_sd));
    distance_profile[distance_profile < 0] = 0;
  } catch (Rcpp::internal::InterruptedException &ex) {
    Rcout << "Process terminated." << std::endl;
...
struct MassWorker : public Worker {
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    for (uint64_t i = 0; i < jump; i++) {
      dp[begin + i] = 2 * (w_size - (z[k - jump + i].real() -
                                     w_size * d_mean[begin + i] * q_mean) /
                                        (d_std[begin + i] * q_std));
...
List mass3_rcpp_parallel(const NumericVector query_window,
  ...
  try {
    ...
    MassWorker mass_worker(data_ref, query_window, window_size, data_size,
                           data_mean, data_sd, query_mean, query_sd,
...
List mass3_rcpp(const NumericVector query_window, const NumericVector data_ref,
  ...
  try {
    for (j = 0; j <= seq_end; j = j + jump) {
      ...
      ComplexVector X = fft_rcpp(data_ref[Range(
  ...
  try {
    for (uint16_t j = 0; j < 10; j++) {
      ...
      for (uint16_t i = 0; i < 10; i++) {
        List nn =
            mass3_rcpp(query_ref[Range(i, i + window_size - 1)], data_ref,
...
List mass_pre_rcpp(const NumericVector data_ref, const NumericVector query_ref,
  ...
  List data_avgsd = fast_avg_sd_rcpp(
      data_ref, window_size); // precompute moving average and SD
  ...
  try {
    if (query_size > 0) {
      List query_avgsd = fast_avg_sd_rcpp(
          query_ref, window_size); // precompute moving average and SD
  ...
  return (List::create(Rcpp::Named("data_fft") = data_fft,
...
List mass_pre_weighted_rcpp(const NumericVector data_ref,
  ...
  NumericVector data_w =
      Re(fft_rcpp(data_fft_w_fft, true)) / data_fft_w_fft.length();
  ComplexVector data2_fft = fft_rcpp(pow(data_padded, 2));
  ComplexVector data2_fft_w_fft = data2_fft * w_fft;
  NumericVector data2_w =
      Re(fft_rcpp(data2_fft_w_fft, true)) / data2_fft_w_fft.length();
  ...
  NumericVector data_pre =
      (sumxw2 - 2 * sumxw * data_mean + sumw * pow(data_mean, 2)) /
      pow(data_sd, 2);

  return (List::create(
      Rcpp::Named("data_fft") = data_fft, Rcpp::Named("data_pre") = data_pre,
      Rcpp::Named("data_size") = data_size,
      Rcpp::Named("window_size") = window_size,
      Rcpp::Named("data_mean") = data_mean, Rcpp::Named("data_sd") = data_sd,
      Rcpp::Named("query_mean") = query_mean,
      Rcpp::Named("query_sd") = query_sd, Rcpp::Named("weight") = weight));
}
