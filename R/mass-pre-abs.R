...
#'   )
#' }
mass_pre_abs <- function(data, query = NULL, window_size) {
  if (is.matrix(data)) {
    data <- as.vector(data)
  }
  ...
  sumx2 <- movsum(data^2, window_size)
  ...
  if (query_size > 0) {
    sumy2 <- movsum(query^2, window_size)
  } else {
    sumy2 <- sumx2
  }

  return(list(
    data_fft = data_fft, window_size = window_size, data_size = data_size, sumx2 = sumx2, sumy2 = sumy2
  ))
}
