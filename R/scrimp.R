...
scrimp <- function(..., window_size, exclusion_zone = getOption("tsmp.exclusion_zone", 1 / 2),
  ...
  nn <- tsmp:::dist_profile(data, data, window_size = window_size)
  ...
  if (pre_scrimp > 0) {
    ...
    for (i in pre_scrimp_idxs) {
      ...
      nn <- tsmp:::dist_profile(data, data, nn, window_size = window_size, index = i)
      ...
      refine_distance[(i + 1):endidx] <-
        sqrt(
          abs(
            2 * (window_size -
              (dotproduct[(i + 1):endidx] - window_size * nn$par$data_mean[(i + 1):endidx] * nn$par$data_mean[(idx_nn + 1):(endidx + idx_diff)]) /
      ...
      if (j == 4) {
        cat("DEBUG i: ", i, "\n")
        cat("DEBUG current_step: ", current_step, "\n")
        cat("DEBUG beginidx: ", beginidx, "\n")
        cat("DEBUG idx_diff: ", idx_diff, "\n")
        cat("DEBUG idx_nn: ", idx_nn, "\n")
        return()
      }
